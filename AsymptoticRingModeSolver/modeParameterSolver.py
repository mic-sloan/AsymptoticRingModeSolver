...
def GetMaterialN(sellmeierCoeffs, wl):
    ...
    Input:
        - sellmeierCoeffs : Dictionary containing the sellmeier coefficients of a given material
        - wl : Wavelength of the field (in um) 
        
    Output:
        - Material refractive index at the provided wavelenght
    """
...
def SolveBentOverlaps(simPars):
    ...
    Input:
        - simPars : Dictionary of parameters for the mode simulation (see inline definitions)
        
    Output:
        - df : Dataframe of mode parameters and computed quantities (see inline definitions)
    """
    # Fixed Parameters
    ...
    data = {
        "wavelength" : wavelength * np.ones(numModes * Ncurve, dtype=float),                                # Wavelength parameter (in um)
        "wg_width" : wg_width * np.ones(numModes * Ncurve, dtype=float),                                    # Waveguide cross-sectional width (in um)
        "wg_thickness" : wg_thickness * np.ones(numModes * Ncurve, dtype=float),                            # Waveguide cross-sectional height (in um)
        "bend_curvature" : np.tile(bendCurvature, numModes),                                                # Bend curvature of the mode (in um^-1)
        "mode_number" : np.repeat(np.arange(1, numModes+1), Ncurve),                                        # Spatial mode number (0 = TE00, 1 = TE01)
        "n_eff" : np.concatenate(n_eff),                                                                    # Effective refractive index 
        "loss" : np.concatenate(loss),                                                                      # Effective radiation attenuation parameter (dB/cm)
        "te_fraction" : np.concatenate(te_frac),                                                            # TE fraction of mode
        "selfOverlapStrength" : np.concatenate([overlapStrength[0, 0, :], overlapStrength[1, 1, :]]),       # Self-overlap strength (in um)
        "crossOverlapStrength" : np.concatenate([overlapStrength[0, 1, :], overlapStrength[1, 0, :]])       # Cross-overlap strength (in um)
        }
...
def MakeDataSet(simPars):
    ...
    Input:
        - simPars : Dictionary of parameters for the mode simulation
    """
...
def ComputeOverlapStrength_selfCoupling(bendCurvature, overlapMat, includeOverlap, maxOrder=4, plotFit=False):
    ...
    Input:
        - bendCurvature : Vector containing the values of the waveguide bends in the same ordering as (overlapMat)
        - overlapMat (nx, ny) : Matrix of overlap values for a given input curvature (nx) and output curvature (ny) used in the fitting
        - includeOverlap (nx, ny) : Mask (same size as (overlapMat)) specifying if the given point should be included in the fit (1) or excluded from the fit (0).
        - (Optional) maxOrder : Maximum order of the polynomial fit (defaults to 4)
        - (Optional) plotFit : If True, makes a plot of the fitted data points. 
    """
...
def Fit3Dpoly(xData, yData, zData, fData, maxOrder=4, lowBiasFactor=[0, 0, 0]):
    ...
    Input:
        - xData : Vector of values corresponding to an indepenedent variable (x)
        - yData : Vector of values corresponding to an indepenedent variable (y)
        - zData : Vector of values corresponding to an indepenedent variable (z)
        - fData : Vector of function values corresponding to f(x, y, z)
        - (Optional) maxOrder : Maximum order of the polynomial fit
        - (Optional) lowBiasFactor : Vector of linear bias factors correponding to [x_bias, y_bias, z_bias]. Large, positive, values bias the fit closer to the smaller values of the independent variable.
        
    Output:
        - fittedPars (nx, ny, nz) : 3D matrix of the resulting fitting coefficients. 
    """
...
def GetFitParameters(dataFiles):
    ...
    Input:
        - dataFiles : Vector of file names containing the data to perform the fit over
        
    Output:
        - fittedPars : dictionary of the fitting parameters for each of the fitted mode properties 
    """
...
def GetFittedFunctionValue(fittingCoeffs, x, y, z):
    """ 
    Computes the value of the fitted function given the fitting parameters in (fittingCoeffs) and the points in (x, y, z) 
    
    Input:
        - fittingCoeffs : Fitted Taylor series coefficient for the given mode property
        - x : Value of the independent x coordinate
        - y : Value of the independent y coordinate
        - z : Value of the independent z coordinate
        
    Output:
        - Value of the fitted function for the given (x, y, z)
    """
    val = 0
